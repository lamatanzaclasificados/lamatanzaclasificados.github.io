    ...
    $('.portfolio-grid').magnificPopup({
        ...
        callbacks: {
            open: function () {
                ...
                if (isSlow || isMedium) {
                    mostrarFallback();
                    setTimeout(() => agregarBotonEnlace(this.currItem), 50);
                    return;
                }

                // Conexión rápida: comportamiento original
            ...
            change: function () {
                ...
                const { isSlow, isMedium } = detectarConexion();
                const delay = (isSlow || isMedium) ? 100 : 50;
                setTimeout(() => agregarBotonEnlace(item), delay);
            },
